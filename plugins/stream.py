...
async def media_receive_handler(bot, message):
    ...
    newtext=f"User: **{message.from_user.mention(style='md')}** Track: **#{str(message.chat.id).replace("-", "c")}** Hash: **#{get_hash(log_msg)}{log_msg.message_id}** Link: **[Hold Me]({short_link})**"
